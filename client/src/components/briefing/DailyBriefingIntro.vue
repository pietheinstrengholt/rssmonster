  ...
  >
    ...
    >
      <template v-if="context">
          ...
          <strong>{{ formatCountLabel(context.islandCount, 'interest island') }}</strong>.
      </template>
    </BriefingContextText>
...
export default {
  ...
  computed: {
    ...
    briefingPeriod() {
      const search = String(this.selectionStore.currentSelection.search || '').toLowerCase();
    ...
    briefingRequestSignature() {
      const revision = Number(this.selectionStore.currentSelection.briefingRevision || 0);
