<template>
  <div class="article-meta">
    ...
    <span v-if="cluster && clusterCountTotal > 1 && eventView !== 'all' && cluster.sourceCount >= 2" class="source-badge" :title="`${cluster.sourceCount} unique sources`"><BootstrapIcon icon="people-fill" class="source-diversity-icon" />{{ cluster.sourceCount }} sources</span>
    <span v-if="cluster && clusterCountTotal > 1 && eventView !== 'all'" class="similar-badge" @click.stop="$emit('view-cluster-articles', cluster.id)">+{{ clusterCountTotal - 1 }} similar article{{ clusterCountTotal - 1 === 1 ? '' : 's' }}</span>
