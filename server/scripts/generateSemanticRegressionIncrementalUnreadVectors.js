...
const FIXTURE_PATH = join(__dirname, '..', 'tests', 'fixtures', 'semantic-regression-incremental.unread.json');
const VECTOR_FIXTURE_PATH = join(__dirname, '..', 'tests', 'fixtures', 'semantic-regression-incremental.unread.vectors.json');
...
function articleTitle(article, articleIndex) {
  ...
  return article.title || firstSentence?.slice(0, 180) || `Semantic incremental unread fixture article ${articleIndex + 1}`;
}
...
async function main() {
  if (!process.env.OPENAI_API_KEY) {
    throw new Error('OPENAI_API_KEY is required to generate semantic regression incremental unread vectors.');
  }
  ...
  for (let index = 0; index < fixture.articles.length; index += BATCH_SIZE) {
    ...
    for (const item of shortInput) {
      console.warn(
        `[SEMANTIC INCREMENTAL UNREAD FIXTURE] embedding short article ${item.articleIndex + 1} ` +
        `(event embedding text length: ${item.embeddingInput.length})`
      );
    }

    console.log(
      `[SEMANTIC INCREMENTAL UNREAD FIXTURE] processed ${Math.min(index + BATCH_SIZE, fixture.articles.length)}` +
  ...
  await writeFile(
    ...
    JSON.stringify({
      embeddingModel: EMBEDDING_MODEL,
      sourceFixture: 'semantic-regression-incremental.unread.json',
      articles: vectorRows
    }, null, 2) + '\n',
    'utf8'
  );

  console.log(`[SEMANTIC INCREMENTAL UNREAD FIXTURE] wrote ${VECTOR_FIXTURE_PATH}`);
}

main().catch(err => {
  console.error('[SEMANTIC INCREMENTAL UNREAD FIXTURE] failed:', err);
  process.exitCode = 1;
});
