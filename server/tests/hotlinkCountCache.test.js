...
describe('hotlink count cache', () => {
  it('counts normalized URLs and query-suffixed hotlinks from other feeds', () => {
    ...
    expect(cache.count('https://example.com/article?campaign=feed', 1)).toBe(2);
    expect(cache.count('https://example.com/article/', 2)).toBe(2);
  ...
  it('indexes hotlinks added during a crawl', () => {
    ...
    cache.add({ feedId: 2, url: 'https://example.com/article?source=feed' });
