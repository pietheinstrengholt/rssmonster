...
import normalizePublisherCards from '../../services/crawl/content/normalizePublisherCards.js';
...
describe('normalizePublisherCards', () => {
  ...
  it.each(['data-src', 'data-lazy-src'])(
    'recovers a Ghost bookmark thumbnail from %s before replacing the card',
    lazyAttribute => {
      const $ = cleanup(
        '<figure class="kg-card kg-bookmark-card">' +
        '<a class="kg-bookmark-container" href="https://example.com/story">' +
        '<div class="kg-bookmark-title">Lazy thumbnail story</div>' +
        '<div class="kg-bookmark-thumbnail"><img ' +
        `${lazyAttribute}="https://cdn.example.com/lazy.jpg"></div>` +
        '</a></figure>'
      );

      expect($('.rss-content-card__image').attr('src')).toBe(
        'https://cdn.example.com/lazy.jpg'
      );
    }
  );

  it('recovers a Ghost bookmark thumbnail from srcset before replacing the card', () => {
    const $ = cleanup(
      '<figure class="kg-card kg-bookmark-card">' +
      '<a class="kg-bookmark-container" href="https://example.com/story">' +
      '<div class="kg-bookmark-title">Responsive thumbnail story</div>' +
      '<div class="kg-bookmark-thumbnail"><img ' +
      'srcset="https://cdn.example.com/small.jpg 480w, ' +
      'https://cdn.example.com/large.jpg 1200w"></div>' +
      '</a></figure>'
    );
