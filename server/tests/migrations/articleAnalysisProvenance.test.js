...
import { up, down } from '../../migrations/20260917000000-add-article-analysis-provenance.mjs';

describe('article analysis provenance migration', () => {
  it('leaves legacy history unknown, supports repeat application and removes only its own column', async () => {
    ...
    try {
      ...
      expect(await query.rawSelect('articles', { where: { id: 1 } }, 'aiAnalysisProvenance')).toBeNull();
      expect(await query.rawSelect('articles', { where: { id: 1 } }, 'interestScore')).toBe(0.5);
      await down(query);
      expect((await query.describeTable('articles')).aiAnalysisProvenance).toBeUndefined();
