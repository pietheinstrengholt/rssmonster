...
describe('crawl priority lease service', () => {
  beforeEach(async () => {
    await db.WorkerLease.destroy({ where: { key: CRAWL_PRIORITY_LEASE_KEY } });
  });

  it('fences ownership until the database-visible lease expires', async () => {
    ...
    const blocked = await acquireCrawlPriorityLease({
      owner: 'crawl-b',
      now: new Date(now.getTime() + 5_000),
      leaseMs: 10_000
    });
    const replacement = await acquireCrawlPriorityLease({
      owner: 'crawl-b',
    ...
    expect(await isCrawlPriorityLeaseActive({
      now: new Date(now.getTime() + 10_002)
    })).toBe(true);
    expect(await releaseCrawlPriorityLease({
      owner: 'crawl-a',
      now: new Date(now.getTime() + 10_003)
    })).toBe(false);
    expect(await releaseCrawlPriorityLease({
      owner: 'crawl-b',
      now: new Date(now.getTime() + 10_003)
    ...
    expect(await isCrawlPriorityLeaseActive({
      now: new Date(now.getTime() + 10_004)
    })).toBe(false);
  });
