...
vi.mock('../../services/score/scoreArticlesFromIslands.js', () => ({ explainArticleInterests: mocked.explainInterests }));
...
describe('dailyBriefing.service', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    mocked.explainInterests.mockResolvedValue({ context: { islands: [] }, results: new Map() });
  ...
  // Builds deterministic summary items from the owned Events and direct Island evidence.
  it('ranks, deduplicates, and enriches morning summary events', async () => {
    const generatedAt = new Date('2026-07-31T12:00:00Z');
    const candidates = [
      { id: 1, eventId: 1, feedId: 10 },
      { id: 2, eventId: 2, feedId: 10 },
      { id: 3, eventId: 3, feedId: 20 },
      { id: 4, eventId: 4, feedId: 30 },
      { id: 5, eventId: 5, feedId: null },
      { id: 6, eventId: null, feedId: 30 }
    ];
    const events = [
      { id: 1, name: '', representativeArticleId: 101, eventStrength: 10, createdAt: generatedAt },
      { id: 2, name: 'Second event', generatedName: 'Generated second event', representativeArticleId: 102, eventStrength: 10, createdAt: '2026-07-20T00:00:00Z' },
      { id: 3, name: 'Third event', representativeArticleId: 103, eventStrength: 9, createdAt: generatedAt },
      { id: 4, name: 'Fourth event', representativeArticleId: 104, eventStrength: 8, createdAt: generatedAt },
      { id: 5, name: 'Duplicate representative', representativeArticleId: 104, eventStrength: 7, createdAt: generatedAt },
      { id: 6, name: 'Missing representative', representativeArticleId: 999, eventStrength: 100, createdAt: generatedAt }
    ];
    ...
    mocked.explainInterests.mockResolvedValue({
      context: { islands: [
        { id: 1, label: 'One', weight: 0.5 },
        { id: 2, label: 'Two', generatedLabel: 'Generated Two', weight: 0.8 },
        { id: 3, label: 'Three', weight: 0.4 }
      ] },
      results: new Map([['101', { paths: [{ islandId: 3 }] }],
        ['102', { paths: [{ islandId: 2 }] }], ['103', { paths: [{ islandId: 1 }] }]])
    });
