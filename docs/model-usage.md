...
article, event, interest-island, taxonomy, and duplicate-detection
...
Events and Interest Islands but deliberately preserves article vectors.
Use the guarded model-rebuild command below when changing models in an existing
environment.

Similarity thresholds are model-dependent. Evaluate events, interest
...
Review the reported article, vector, event, island, and taxonomy counts.
...
6. Rebuilds duplicate detection, historical events, event and behavioral
   Islands and article interest scores.
