...
| `npm run start:worker` | None | Starts the dedicated crawl worker. It runs an iteration immediately, waits for `CRAWL_WORKER_INTERVAL_MS`, and repeats. Use one supervised worker in production; see [Crawling](crawling.md). |
