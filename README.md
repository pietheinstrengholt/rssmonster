...
- **Advanced Search Expressions**: Composable filters using field operators (`star:true`, `unread:false`, `read:true`, `clicked:true`, `seen:false`, `hot:true`, `cluster:eventCluster`, `cluster:topicGroup`, `tag:tech`, `title:javascript`), article age filters (`firstSeen:24h`, `firstSeen:7d`), quality & freshness thresholds (`quality:>0.6`, `freshness:>=0.5`), sorting (`sort:ASC`, `sort:DESC`, `sort:IMPORTANCE`, `sort:QUALITY`, `sort:ATTENTION`), and flexible date filters (`@2025-12-14`, `@today`, `@yesterday`, `@lastweek`, `@"3 days ago"`, `@"last Monday"`). Example: `title:javascript ai @today quality:>0.6 sort:IMPORTANCE`
- **Smart Folders**: Smart Folders allow you to create declarative, dynamic views of your content using composable search expressions. Examples: `@today unread:true cluster:eventCluster sort:IMPORTANCE` (Top Stories Today), `unread:true quality:>0.7 cluster:eventCluster sort:QUALITY` (Worth Your Time), `unread:true cluster:topicGroup quality:>0.8 freshness:>=0.5 sort:IMPORTANCE` (Low Noise Mode).
